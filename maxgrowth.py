...
TREND_REGIMES = ["strong_bull", "weak_bull", "neutral", "weak_bear", "strong_bear"]
...
class MarketRegimeDetector:
    ...
    def _calculate_technical_indicators(self):
        ...
        trend_components = [
            0.3 * (self.daily_data['sma_50'] > self.daily_data['sma_200']),
            0.2 * (self.weekly_data['sma_10'] > self.weekly_data['sma_40']).resample('D').ffill(),
            0.2 * (self.daily_data['adx'] / 100),
            0.1 * (self.daily_data['macd'] > self.daily_data['macd_signal']),
            0.1 * (self.daily_data['c'] > self.daily_data['sma_50']),
            0.1 * (self.daily_data['rsi_14'] / 100)
        ]
    ...
    async def get_current_regime(self) -> Tuple[str, str]:
        ...
        trend_regime = max(
            ((r, regimes[r]) for r in TREND_REGIMES),
            key=lambda x: x[1],
            default=("neutral", 0)
        )[0]
        
        vol_regime = max(
            ((r, regimes[r]) for r in VOLATILITY_REGIMES),
            key=lambda x: x[1],
    ...
    async def get_regime_description(self) -> Dict:
        ...
        return {
            ...
            "trend_probabilities": {str(r): float(regimes[r]) for r in TREND_REGIMES},
            "volatility_probabilities": {str(r): float(regimes[r]) for r in VOLATILITY_REGIMES},
            "transition_analysis": {
    ...
    def _normalize_regime_probabilities(self, regimes: Dict[str, float]) -> Dict[str, float]:
        trend_total = sum(regimes[r] for r in TREND_REGIMES)
        vol_total = sum(regimes[r] for r in VOLATILITY_REGIMES)
        
        normalized = {}
        for r in regimes:
            if r in TREND_REGIMES:
...
class StockScanner:
    ...
    def _calculate_composite_score(self, data: pd.DataFrame, indicators: Dict, 
        ...
        norm_momentum = min(max(indicators['momentum'] / 30, 0), 1)
        norm_volume = min(max(indicators['volume_ma'] / criteria["min_volume"], 0), 2)
        norm_volatility = min(max((indicators['volatility'] - 0.2)/0.3, 0), 1)
        norm_patterns = min(len(patterns) / 3, 1)
        ...
        for strategy, config in self.strategy_config.items():
            ...
            if current < min_target:
                candidates = sorted(
                    ...
                    self._score_strategy(s, strategy) > 0.4],
    ...
    def next(self):
        ...
        exit_long = (
            self.position and (
                ...
                self.rsi[-1] < 45 or
                price > self.position.price * 1.20 or
...
async def optimize_hypergrowth_params(df: pd.DataFrame):
    """Optimize hypergrowth strategy parameters using grid search"""
    best_score = -np.inf
    best_params = None
