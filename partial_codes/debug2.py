...
class MarketRegimeAnalyzer:
    ...
    def generate_test_market_data(self):
        ...
        for i in range(1, len(dates)):
            ...
            if regime_duration <= 0 or random.random() < 0.05:
                current_regime = random.choice(["Severe Bear", "Bear", "Neutral", "Bull", "Strong Bull"])
                regime_duration = random.randint(20, 60)
...
class SmartStopLoss:
    def __init__(self, entry_price, atr, adx, market_volatility=None, regime=None,
                 ...
                 creation_time=None):  # Add creation_time parameter
        self.entry_price = entry_price
        self.atr = atr
        self.adx = adx
        self.regime = regime or "Neutral"
        self.creation_time = creation_time or datetime.now(tz.utc)  # Use provided or current UTC time
    ...
    def update_trailing_stop(self, current_price, timestamp):
        ...
        # Calculate holding time using creation_time
        holding_days = (timestamp - self.creation_time).days
        
        new_stop = current_price - self.atr * self.volatility_factor * sensitivity
        
        # Adjust stop based on holding time
...
class TradingSystem(QThread):
    ...
    log_signal = pyqtSignal(str, datetime)  # Modified to include timestamp
    ...
    def log(self, message):
        ...
        if self.backtest_mode:
            ...
            timestamp = datetime.now(tz.utc)
        self.log_signal.emit(message, timestamp)
    ...
    def force_scan(self):
        ...
        self.log("Manual scan requested by user")
    ...
    def run_backtest(self):
        ...
        self.log(f"Backtest started from {self.start_date} to {self.end_date}")
    ...
    def periodic_backtest_evaluation(self):
        ...
        try:
            ...
            self.log("Backtest interrupted by user")
            ...
            self.log(f"Backtest evaluation error: {str(e)}")
            self.stop_system()
    ...
    def daily_update(self):
        for ticker in self.positions.keys():
            if ticker in self.lookback_system.position_age:
                ...
                self.lookback_system.position_age[ticker] = 1
        self.log("Daily position age updated")
    ...
    def update_market_regime(self):
        ...
        self.log("Starting market regime analysis in background...")
    ...
    def handle_regime_result(self, result):
        ...
        self.log(f"Market regime updated: {self.market_regime} (Volatility: {self.market_volatility*100:.2f}%)")
    ...
    def send_discord_notification(self):
        ...
        try:
            ...
            if response.status_code != 204:
                self.log(f"Discord notification failed: {response.status_code} {response.text}")
            else:
                self.log("Sent market analysis to Discord")
    ...
    def evaluate_opportunities(self):
        ...
        if not tickers_to_score:
            self.log("No tickers to evaluate (all in positions)")
            return
    ...
    def enter_top_opportunities(self):
        if not hasattr(self, 'scoring_results') or not self.scoring_results:
            self.log("No opportunities to enter")
            return
        ...
        for opp in self.last_opportunities[:1]:
            if opp['score'] >= self.MIN_SCORE_FOR_ENTRY and opp['ticker'] not in self.positions:
                ...
                if not data:
                    self.log(f"No data for {opp['ticker']}")
                    continue
                    
                df = self.data_handler.get_historical(opp['ticker'], 50)
                if df.empty:
                    self.log(f"Insufficient data for {opp['ticker']}")
                    continue
                ...
                try:
                    ...
                    self.log(f"ADX calculation failed for {opp['ticker']}: {str(e)}")
                    adx = 20.0
                ...
                if hasattr(self, 'market_volatility') and hasattr(self, 'market_regime'):
                    stop_system = SmartStopLoss(
                        ...
                        regime=self.market_regime,
                        creation_time=self.clock.current_time if self.backtest_mode else datetime.now(tz.utc)  # Use clock time
                    )
                else:
                    stop_system = SmartStopLoss(
                        entry_price=current_price,
                ...
                if opp['ticker'] in self.positions:
                    self.log(f"Already in position for {opp['ticker']}")
                else:
                    self.log(f"Score too low for {opp['ticker']}: {opp['score']} < {self.MIN_SCORE_FOR_ENTRY}")
        ...
        try:
            ...
            for ticker, pos in self.positions.values():
                try:
                    ...
                    self.log(f"Error processing position {ticker}: {str(e)}")
            ...
            self.log(f"CRITICAL: State generation failed: {str(e)}")
            return {
                'timestamp': datetime.now(tz.utc).strftime('%Y-%m-%d %H:%M:%S UTC'),
                'market_open': False,
                'capital': self.capital,
    ...
    def score_trade_opportunity(self, ticker):
        ...
        try:
            ...
            # 2. Determine if we're evaluating an open position
            is_open_position = ticker in self.positions
            
            # 3. Get base data (35 days) for initial ADX calculation
            ...
            if base_data is None or base_data.empty:
                self.log(f"Insufficient base data for {ticker}")
                return None
                
            # 4. Calculate base ADX from base_data
            try:
                ...
                self.log(f"Base ADX calculation failed for {ticker}: {str(e)}")
                base_adx = 20.0
                
            # 5. Get adaptive lookback period
            ...
            # 6. Get data for the adaptive lookback period
            ...
            if data is None or data.empty:
                self.log(f"Insufficient data for adaptive lookback on {ticker}")
                return None
                
            # 7. Calculate indicators with adaptive lookback data
            ...
            try:
                ...
                self.log(f"ADX calculation failed for {ticker}: {str(e)}")
                adx = 20.0  # Default value
                
            # RSI (Relative Strength Index)
            rsi = self.ensure_scalar(data.ta.rsi(length=14).iloc[-1])
            
            # 8. Get latest price and volume data
            latest = self.data_handler.get_latest(ticker)
            if not latest:
                self.log(f"No latest data for {ticker}")
                return None
            price = latest['close']
            volume = latest['volume']
            
            # 9. Calculate average volume
            ...
            # 10. Create stop system for scoring
            if hasattr(self, 'market_volatility') and hasattr(self, 'market_regime'):
                stop_system = SmartStopLoss(
                    entry_price=price,
                    atr=atr,
                    ...
                    market_volatility=self.market_volatility,
                    regime=self.market_regime,
                    creation_time=self.clock.current_time if self.backtest_mode else datetime.now(tz.utc)
                )
            else:
            ...
            # 11. Calculate component scores
            # Trend Strength Score (ADX-based)
            adx_score = min(100, max(0, (adx - 20) * 5))  # 20-40 ADX → 0-100 score
            
            # Volatility Quality Score (ATR-based)
            atr_pct = atr / price
            ...
            rr_score = min(100, rr_ratio * 25)  # 4:1 ratio = 100 score
            
            # Volume Confirmation Score
            volume_ratio = volume / avg_volume
            volume_score = min(100, volume_ratio * 50)  # 2x volume = 100 score
            ...
            if rsi > 70:  # Overbought
                ...
                rsi_score = 80 - abs(rsi - 50)  # 50 RSI = best score
                
            # 12. Sector Strength Adjustment
            ...
            # 13. Market Regime Adjustment
            regime_factor = self.get_regime_factor()
            
            # 14. Weighted composite score
            composite_score = (
                0.30 * adx_score + 
                0.25 * atr_score + 
                0.20 * rr_score + 
            ...
            # 16. Log the score with details (as requested)
            self.log(
                f"Scored {ticker}: {composite_score:.1f} "
                f"(Lookback: {lookback_days}d, "
                f"Sector: {sector} {sector_score:.1f})"
            )
            
            # 17. Return scoring results
            return {
                'ticker': ticker, 
            ...
            self.log(f"Scoring error for {ticker}: {str(e)}")
            return None
    ...
    def enter_position(self, ticker, price, atr, adx, original_score, stop_system):
        try:
            if ticker in self.positions:
                self.log(f"Already in position for {ticker}")
                return
                
            risk_per_share = price - stop_system.initial_stop
            if risk_per_share <= 0:
                self.log(f"Invalid risk for {ticker}: risk_per_share={risk_per_share}")
                return
                
            position_size = math.floor((self.capital * self.risk_per_trade) / risk_per_share)
            if position_size <= 0:
                self.log(f"Invalid position size for {ticker}: {position_size}")
                return
                
            # Use backtest clock time if in backtest mode
            entry_time = self.clock.current_time if self.backtest_mode else datetime.now(tz.utc)
            ...
            self.log(f"Entered {ticker} at ${price:.2f} - {position_size} shares")
        except Exception as e:
            self.log(f"Entry error for {ticker}: {str(e)}")

    def exit_position(self, ticker, exit_price, reason):
        try:
            if ticker not in self.positions:
                self.log(f"Exit failed: no position for {ticker}")
                return
            ...
            self.log(f"Exit error for {ticker}: {str(e)}")

    def partial_exit(self, ticker, percent, exit_price, reason):
        try:
            if ticker not in self.positions:
                self.log(f"Partial exit failed: no position for {ticker}")
                return
                
            position = self.positions[ticker]
            if percent <= 0 or percent >= 1:
                self.log(f"Invalid partial exit percent for {ticker}: {percent}")
                return
                
            shares_to_sell = math.floor(position['shares'] * percent)
            if shares_to_sell <= 0:
                self.log(f"Invalid shares to sell for {ticker}: {shares_to_sell}")
                return
            ...
            if position['shares'] <= 0:
                ...
                self.log(f"Fully exited {ticker} via partial exits")
                
        except Exception as e:
            self.log(f"Partial exit error for {ticker}: {str(e)}")
    
    def evaluate_and_replace_positions(self):
        if not self.positions or not self.runner_ups:
            return
            
        self.log("Evaluating position strength...")
    ...
    def calculate_current_score(self, ticker, position, current_price):
        try:
            ...
            self.log(f"Score calculation failed for {ticker}: {str(e)}")
            return 0
    
    def find_replacement(self, weak_ticker, weak_score, current_price):
        self.log(f"Seeking replacement for {weak_ticker} (Score: {weak_score:.1f})")
        best_candidate = None
        ...
        if best_candidate:
            self.log(f"Replacing {weak_ticker} with {best_candidate['ticker']}")
            self.execute_replacement(weak_ticker, best_candidate)
    
    def find_replacement_from_scan(self, min_score):
        self.log("Scanning for new replacement candidates...")
        opportunities = []
        for ticker in self.tickers:
            if ticker in self.positions or ticker in self.executed_tickers:
                continue
        ...
        if position:
            ...
            if new_data:
                ...
                try:
                    ...
                    self.log(f"ADX calculation failed for {new_candidate['ticker']}: {str(e)}")
                    adx = 20.0
                ...
                self.log(f"Replaced {old_ticker} with {new_candidate['ticker']}")
    ...
    def stop_system(self):
        if self.running:
            self.log("Stopping trading system...")
            self.running = False
...
class TradingDashboard(QMainWindow):
    def __init__(self, testing_mode=False):
        ...
        self.log_message("Trading Dashboard Initialized", datetime.now(tz.utc))
        self.log_message(f"Tracking Tickers: {', '.join(self.tickers)}", datetime.now(tz.utc))
        self.log_message(f"Operating in {'TESTING' if testing_mode else 'LIVE'} mode", datetime.now(tz.utc))
        self.log_message("Press 'Start System' to begin trading", datetime.now(tz.utc))
    ...
    def log_message(self, message, timestamp=None):
        if timestamp is None:
            timestamp = datetime.now()
        
        # Format timestamp based on mode
        if hasattr(self, 'trading_system') and self.trading_system and self.trading_system.backtest_mode:
            # Backtest mode - show full date and time
            timestamp_str = timestamp.strftime("%Y-%m-%d %H:%M:%S")
        else:
            # Live mode - just show time
            timestamp_str = timestamp.strftime("%H:%M:%S")
        
        self.terminal_output.append(f"[{timestamp_str}] {message}")
    ...
    def start_system(self):
        try:
            ...
            self.log_message("Invalid capital or risk value. Using defaults.", datetime.now(tz.utc))
            capital = 100000
            risk = 0.01
        ...
        self.trading_system.log_signal.connect(lambda msg, ts: self.log_message(msg, ts))
        ...
        if not self.trading_system.isRunning():
            if self.trading_system.start_system():
                ...
                self.log_message("Trading system started", datetime.now(tz.utc))
                return
        self.log_message("System already running", datetime.now(tz.utc))
    ...
    def stop_system(self):
        if self.trading_system and self.trading_system.isRunning():
            if self.trading_system.stop_system():
                ...
                self.stop_button.setEnabled(False)
                self.scan_button.setEnabled(False)
                self.log_message("Trading system stopped", datetime.now(tz.utc))
                return
        self.log_message("System not running", datetime.now(tz.utc))
    
    def request_scan(self):
        if self.trading_system and self.trading_system.running:
            self.trading_system.scan_requested.emit()
            self.log_message("Manual scan requested", datetime.now(tz.utc))
        else:
            self.log_message("System not running - cannot scan", datetime.now(tz.utc))
    ...
    def update_ui(self, state):
        ...
        try:
            ...
            self.log_message(f"UI update error: {str(e)}", datetime.now(tz.utc))
    ...
    def run_backtest(self):
        ...
        if self.trading_system.start_system():
            ...
            self.stop_button.setEnabled(True)
            self.scan_button.setEnabled(True)
            self.log_message("Backtest started", datetime.now(tz.utc))
    ...
    def update_backtest_results(self, state):
        ...
        try:
            ...
            self.log_message(f"Backtest results error: {str(e)}", datetime.now(tz.utc))
    ...
    def update_equity_curve(self, equity_data):
        ...
        try:
            ...
            self.log_message(f"Equity curve error: {str(e)}", datetime.now(tz.utc))
