...
class TradingSystem:
    ...
    async def place_bracket_order(self, ticker, qty, row):
        ...
        try:
            ...
            if response.status_code != 200:
                error_msg = f"Buy order failed for {ticker}: {response.text}"
                print(error_msg)
                await self.notifier.send_system_alert(error_msg, is_error=True)
                return False
            ...
            # 2. Place trailing stop order
            trailing_stop_order = TrailingStopOrderRequest(
                symbol=ticker,
                qty=qty,
                side=OrderSide.SELL,
                trail_percent=3.0,  # Trail by 3% - adjust based on your strategy
                time_in_force=TimeInForce.GTC
            )
            trailing_stop_resp = trading_client.submit_order(trailing_stop_order)
            print(f"Placed trailing stop for {ticker}")
            
            # 3. Place hard stop order
            ...
            hard_stop_resp = trading_client.submit_order(hard_stop_order)
            print(f"Placed hard stop at ${bracket_details['hard_stop']['stop_price']:.2f}")
            
            # Store order IDs for later management
            order_ids = {
                "buy": buy_order["id"],
                "trailing_stop": trailing_stop_resp.id,
                "hard_stop": hard_stop_resp.id
            }
            
            # Update position with order IDs
            if ticker in self.active_positions:
                self.active_positions[ticker]["order_ids"] = {
                    "stop": trailing_stop_resp.id,
                    "hard_stop": hard_stop_resp.id
                }
