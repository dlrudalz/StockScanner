...
class Debugger:
    ...
    def _setup_logging(self):
        ...
        if not self.enabled:
            # Completely disable logging if not enabled
            logging.getLogger().addHandler(logging.NullHandler())
            self.logger = logging.getLogger(__name__)
            self.logger.disabled = True
            return
            
        # Configure proper logging if enabled
    ...
    def log_dataframe(self, df: pd.DataFrame, message: str = "", level: str = "DEBUG"):
        ...
        if len(df) <= 10:  # Only show full data for small DataFrames
            self.log("\n" + str(df), level)
        else:
            self.log(f"\nFirst 5 rows:\n{df.head()}\n\nLast 5 rows:\n{df.tail()}", level)
...
class StockScanner:
    ...
    def __init__(
        ...
        debugger: Debugger,
        max_tickers_to_scan: Optional[int] = 100  # None for all tickers, or specific number
    ):
        self.polygon = polygon_client
...
class FixedHypergrowthStrategy(Strategy):
    """Improved hypergrowth strategy with better entry conditions"""
    def init(self):
    ...
    def next(self):
        ...
        # Stricter but more reliable entry conditions
        enter_long = (
            price > self.sma50[-1] * 1.05 and  # Price 5% above SMA50
            self.rsi[-1] > 55 and
            self.rsi[-1] < 75 and  # Avoid overbought
            self.macd[-1] > self.macd_signal[-1] and  # MACD confirmation
            self.data.Volume[-1] > self.volume_ma[-1] * 1.5 and
            not self.position and
            len(self.data) > 100  # Minimum data points
        )
        
        # More active profit taking
        exit_long = (
            self.position and (
                price < self.sma20[-1] or
                self.rsi[-1] < 45 or
                price > self.position.price * 1.20 or  # Take partial profits at 20%
...
class FixedMomentumStrategy(Strategy):
    """Improved momentum strategy with better signals"""
    def init(self):
        self.sma50 = self.I(talib.SMA, self.data.Close, 50)
    ...
    def next(self):
        ...
        # Enhanced entry with multiple confirmations
        enter_long = (
            crossover(self.sma50, self.sma200) and
            self.adx[-1] > 25 and
            self.rsi[-1] > 50 and  # Avoid oversold conditions
            price > self.sma50[-1] * 1.02 and  # Price 2% above SMA50
            not self.position and
            len(self.data) > 200  # Ensure enough data points
        )
        
        # More dynamic exit conditions
        exit_long = (
            self.position and (
                crossover(self.sma200, self.sma50) or
                price < self.sma50[-1] or
                self.rsi[-1] > 70 or  # Take profits when overbought
                price < self.position.price * 0.95  # 5% trail stop
            )
        )
        
        if enter_long:
            self.buy(sl=price - 2.5*atr)  # Wider stop for volatility
        elif exit_long and self.position:
            self.position.close()

class FixedBreakoutStrategy(Strategy):
    """Enhanced breakout strategy with better signal detection"""
    resistance_window = 20
    support_window = 20
    min_volume_multiplier = 1.5
    
    def init(self):
        # Calculate rolling max/min directly from the data
        self.resistance = self.I(self._calculate_resistance, self.data)
        self.support = self.I(self._calculate_support, self.data)
        self.volume_ma = self.I(talib.SMA, self.data.Volume, 20)
        self.atr = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, 14)
        self.rsi = self.I(talib.RSI, self.data.Close, 14)
        self.adx = self.I(talib.ADX, self.data.High, self.data.Low, self.data.Close, 14)
    ...
    def _calculate_resistance(self, data):
        """Calculate rolling resistance (high)"""
        return data.High.rolling(self.resistance_window).max()
        
    def _calculate_support(self, data):
        """Calculate rolling support (low)"""
        return data.Low.rolling(self.support_window).min()

    def next(self):
        current_close = self.data.Close[-1]
        current_volume = self.data.Volume[-1]
        
        resistance_break = current_close > self.resistance[-1]
        volume_spike = current_volume > self.volume_ma[-1] * self.min_volume_multiplier
        trend_confirmation = self.adx[-1] > 20
        
        support_break = current_close < self.support[-1]
        ...
        elif self.position and (support_break or rsi_overbought):
            self.position.close()
            
        if self.position and current_close > self.position.price * 1.05:
...
async def run_backtest(stock_data: Dict, debugger: Debugger) -> Optional[Dict]:
    """Fixed backtest function with robust data preparation"""
    try:
        # 1. Validate input structure
        if not stock_data or 'data' not in stock_data:
            debugger.error("Invalid stock_data - missing 'data' key")
            return None

        # 2. Prepare the DataFrame with proper column names and types
        try:
            ...
            if len(df) < 20:
                debugger.warning(f"Insufficient data points ({len(df)}) for backtest")
                return None
                
        except Exception as e:
            debugger.error(f"Data preparation failed: {str(e)}")
            return None

        # 3. Select the appropriate strategy class
        ...
        # 4. Run the backtest with proper error handling
        try:
            bt = Backtest(
                df,
                strategy_class,
            ...
            if not results or '# Trades' not in results:
                debugger.error("Invalid backtest results structure")
                return None
                
            # Log basic results
            debugger.info(
                f"Backtest results for {stock_data['ticker']} ({strategy_name}):\n"
                f"Period: {df.index[0].date()} to {df.index[-1].date()}\n"
                f"Trades: {results['# Trades']}\n"
                f"Return: {results['Return [%]']:.1f}%\n"
            ...
            debugger.error(f"Backtest execution failed: {str(e)}")
            return None
            
    except Exception as e:
        debugger.error(f"Backtest setup failed: {str(e)}")
        return None
